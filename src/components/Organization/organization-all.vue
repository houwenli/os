...
<template>
  <div class="ws-organization-options">
    ...
    <el-select v-if="showSearch" v-model="selectValue" v-bind="$attrs" filterable remote reserve-keyword :placeholder=placeholder2 :remote-method="remoteMethod" @change="handleSelect"
      ...
      <el-option v-for="item in options" :key="item[valueKey]" :label="item[nameKey]" :value="item[valueKey]"></el-option>
    </el-select>
  </div>
</template>
...
export default {
    ...
    props: {
        ...
        nameKey: { type: String, default: 'name' },  // 机构名称字段
        ...
        reqData: { type: Object, default: () => { return {} } }
    },
    computed: {
        orgOptions() {
    ...
    methods: {
        ...
        remoteMethod(queryString) {
            ...
            if (queryString !== '' && queryString.length >= 1) {
                ...
                params.op = this.reqData.op || 'selectOrgByNameAndType';
                let URL = this.reqData.moduleName ? this.$getModuleUrl(this.reqData.moduleName, params.op) : '/org/ranking/select';
