                ...
                <div :class="['grid-content-center', item.isinput ? 'ma-tp' : '']" v-for="(item, index) in pageFileList" :key="index">
                    ...
                    <main>
                        <section>
                            ...
                            <el-input style="width: 167px; margin-left: 10px" v-model="item.fileName" v-else @blur="saveFileName(item)" maxlength="40"></el-input>
                            ...
                            <div class="ma-lf-42 color-blue">
                                <span v-if="isShowPreview">
                                    ...
                                    <a
                                        class="color-blue"
                                        :href="item.fileUrl"
                                        v-else-if="isShowImgPreview(item)"
                                        target="_blank"
                                    >
                                        预览
                                    </a>
                                ...
                                <span v-if="isShowRename" class="ma-lf-16">
                                    <!-- <button :disabled="item.isinput" :class="[item.isinput ? 'disabled-btn' : '']">重命名</button> -->
                                    <button v-if="!item.isinput" @click.stop.prevent="handleRename(item)" :class="[item.isinput ? 'disabled-btn' : '']">重命名</button>
                                    <button v-else @click.stop.prevent="handleRanameText(item)">重命名</button>
                                </span>
...
export default {
    ...
    computed: {
        uploadFile() {
            return `${process.env.VUE_APP_NEED_API}/project/docManagement/uploadFile`;
            // 地址需替换为文件服务器地址
    ...
    methods: {
        ...
        // 图片预览是否显示
        isShowImgPreview(item){
            if(!item.fileUrl) return false
            if(item.fileType) {
                return item.fileType.indexOf('pdf') > -1 || item.fileType.indexOf('png') > -1 || item.fileType.indexOf('jpeg') > -1
            }else{
                // 使用ws-image-upload组件上传图片 没有fileType字段
                return true
            }
        },
