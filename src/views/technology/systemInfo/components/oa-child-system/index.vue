<template>
    <div>
        <ws-table-el ref="table" v-bind="tableConfig" :col="8">
            ...
            <template #options>
              <div style="float: left" v-if='powerList.syncSystemMenusFromOA'>
                <el-button type="primary" @click="onOpenSyncModal">同步</el-button>
              </div>
        ...
        <!-- 系统同步的弹窗 -->
        <syncSystemModal v-if='syncModal' :value.sync='syncModal' @updateData='search(2)'></syncSystemModal>
    </div>
</template>

<script>
import syncSystemModal from '../syncSystemModal/index.vue'
...
export default {
    ...
    methods: {
        ...
        changePath(type, row) {
            this.$router.push({
                name: "systemDetail",
                query: { type, id: row ? row.id : null, sysType: '1' }
            });
        },
