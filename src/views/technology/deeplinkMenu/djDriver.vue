    ...
    <div>
        <ws-table-el ref="table" v-bind="tableConfig" :col="8">
            <template #search>
                <search-group>
                    <label for>状态：</label>
                    <el-select v-model="searchOptions.openStatus" placeholder="请选择使用状态" clearable>
            ...
            <template>
                ...
                <ws-table-column-el prop="deepLinkUrl" label="Deeplink路径"></ws-table-column-el>
                <ws-table-column-el prop="openStatus" label="状态">
                    <template slot-scope="scope">
                        <div v-if="scope.row.openStatus == 1" class="start">启用</div>
                        <div v-if="scope.row.openStatus == 2" class="stop">停用</div>
                    </template>
                </ws-table-column-el>
                <ws-table-column-el fixed="right" label="操作" width="200px">
                    <template slot-scope="scope">
                        ...
                        <el-button type="text" v-if="oaPowerList.update" @click="openDialog('update', scope.row.id)" style="color: #0088ff">编辑</el-button>
                        <el-button type="text" v-if="oaPowerList.updateStatus" @click="updateStatus(scope.row)" style="color: #0088ff">
                            {{ `${scope.row.openStatus == 1 ? '停用' : '启用'}` }}
                        </el-button>
                        <el-button type="text" v-if="oaPowerList.updateStatus && scope.row.openStatus == 2" @click="deleteOp(scope.row.id)">删除</el-button>
                    </template>
                </ws-table-column-el>
            </template>
        </ws-table-el>
...
export default {
    ...
    methods: {
        updateStatus(row) {
            this.$confirm(`您确定${row.openStatus == 1 ? '停用' : '启用'}此Deeplink吗？`, '提示', {
        ...
        deleteOp(id) {
            this.$confirm('您确定删除此Deeplink吗？', '提示', {
                confirmButtonText: '确定',
                cancelButtonText: '取消',
                center: true
            }).then(() => {
