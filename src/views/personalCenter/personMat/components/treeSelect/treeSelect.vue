        ...
        <div class="mechanism">
            <div class="rowDiv">
                <div class="leftDiv">
                    ...
                    <div class="leftBody">
                        <!-- <el-autocomplete clearable style="width: 100%;margin-bottom:10px;" :popper-class="autoClass" v-model="searchVal" :debounce="300" :fetch-suggestions="querySearch" placeholder="请输入名字" @select="handleSelect">
                            <i class="el-icon-search el-input__icon" slot="prefix" @click="handleIconClick"></i>
                            <template slot-scope="{ item }">
                                <div class="content">
                                    <div>{{item.noName}}</div>
                                    <div class="have-addr">{{ item.name }}</div>
                                    <div class="addpart"> 我是公司前端小开发 </div>
                                    <div class="adddep">前端部</div>
                                </div>
                            </template>
                        </el-autocomplete> -->
                        <el-select
                        ...
                        >
                            <el-option v-for="(item, index) in selectOpts" :key="index" :label="item.name" :value="item.id">
                        ...
                        <div align="left">
                            <div class="breadcrumb" v-if="breadcrumbList.length > 1">
                                ...
                                <!-- <el-breadcrumb separator="/">
                                    <el-breadcrumb-item v-for="item in breadcrumbList" :key="item.id">{{ item.label }}</el-breadcrumb-item>
                                </el-breadcrumb> -->
                                <div>
                                    <span v-for="(item, index) in breadcrumbList" :key="index" class="el-breadcrumb__item">
                        ...
                        <el-tree
                            ...
                            class="filter-tree"
                            :style="{
                                height: breadcrumbList.length > 1 ? (isCheckAllVisiable ? 'calc(100% - 150px)' : 'calc(100% - 90px)') : 'calc(100% - 50px)'
                            }"
                        ...
                        >
                            <span class="custom-tree-node" slot-scope="{ data }">
                                <span class="bigSpan" v-if="data.type === 'staff'">
                                    {{ data.name }}
                                    <el-tooltip
                                        :disabled="data.partimeString == null || data.partimeString.length == 0"
                                        class="item"
                                        effect="dark"
                                        placement="top-end"
                                    >
                                        ...
                                        <span style="padding: 0 10px 0 5px" class="leftSpan">
                                            {{
                                                data.partimeString
                                                    ? `(${data.partimeString.length > 15 ? `${data.partimeString.substring(0, 15)}...` : data.partimeString})`
                                                    : ''
                                            }}
                                        </span>
                                    </el-tooltip>
                ...
                <div class="rightDiv">
                    ...
                    <div class="cardDiv">
                        <el-card shadow="never" v-for="(item, index) in checkedNodeList" :key="index">
                            <div class="content">
                                <span>{{ item.name }}</span>
                                <img src="./icons/icon_off.png" @click="removeCheckedNode(item, index)" alt="" />
                                <!-- <i class="el-icon-error" @click="removeCheckedNode(item, index)"></i> -->
                            </div>
                        </el-card>
                    </div>
                </div>
...
export default {
    ...
    props: {
        ...
        depType: { type: Number, default: null },
    ...
    methods: {
        ...
        handleSelect(item) {
            // 判断是否有选中的id 有则删除
            let index = this.checkedNodeList.map(i => i.id).indexOf(item[0]);
            if (index >= 0) {
        ...
        async getTreeData(id, type) {
            try {
                ...
                const param = {
                    data: { organizationalid: id, departmentAo: { depType: this.depType }, statusList: [1, 2] },
        ...
        //根据当前节点获取其下所有的叶子节点
        /* getleafNodesFromCurentNode(curentNode) {
            let leafNodeArray = [];
            let getLeafNode = function (node, leafNode) {
                if (node instanceof Array) {
                    node.map(item => {
                        if (item.isLeaf) {
                            item.data.checked = item.checked;
                            leafNode.push(item.data);
                        } else if (item.childNodes && item.childNodes.length) {
                            getLeafNode(item.childNodes, leafNode);
                        }
                    });
                } else {
                    if (node.isLeaf) {
                        node.data.checked = node.checked;
                        leafNode.push(node.data);
                    } else if (node.childNodes && node.childNodes.length) {
                        getLeafNode(node.childNodes, leafNode);
                    }
                }
            };
            getLeafNode(curentNode, leafNodeArray);
