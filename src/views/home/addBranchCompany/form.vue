    ...
    <div>
        <div class="org-branch-company">
            ...
            <validate-form
                ref="validateForm"
                :pageType="pageType"
                :ruleForm="branchComForm"
                :rpcCompanyConsoleAo="rpcCompanyConsoleAo"
                :rules="branchComRules"
            ...
            <div>
                <div class="pic-list" v-for="(item, index) in attachmentList" :key="index">
                    <ws-image-upload :key="imageUpateKey" ref="imageUpload" class="image-upload" :urlList='handleAttachmentUrl(item)' v-bind="options" :quantity="1" :afterUpload="(...arg) => afterUpload(...arg, item)"></ws-image-upload>
        ...
        <div class="org-branch-company process-box">
            ...
            <audit-process @update:load="updateLoad" class="audit-process" title="" ref="auditProcess" :actions="actions" :query="{ nameCode: 30 }"></audit-process>
        </div>

        <!-- 底部导航 -->
        <footer-fixed>
            <ws-throttle>
                <el-button :loading="btnLoading"  :disabled="disabled" type="primary" style="width: 160px; height: 36px" @click="submitForm('validateForm')">提交审核</el-button>
            </ws-throttle>
        </footer-fixed>
    </div>
</template>
...
import { branchComForm, branchComRules } from '../utils/addOrgForm.js';
...
export default {
    ...
    data() {
        return {
            ...
            rpcCompanyBusinessAo: {}, //新增机构信息
            rpcCompanyConsoleAo: {}, //营业执照信息
            timer: '',
            resubmissionData: {}, //重新提交路由参数
            attachmentList: [], //附件列表
            pictureList: [],
            imgList: [],
            mapShow: false,
            ...
            hasDetail: false,
            imageUpateKey: +new Date()
        };
    },
    computed: {
        marker() {
            const { longitude, latitude } = this.branchComForm;
    ...
    async created() {
      try {
        let res = await this.getAttachmentNameList({ attachmentType: 20 })
       ...
       */
        ...
        submitForm(formName) {
            this.$refs[formName].validate(valid => {
                if (valid) {
                    ...
                    let formData = JSON.parse(JSON.stringify(this.branchComForm));
                    // rpcOrgBusinessAo 营业执照
                    // rpcPersonChargeInfoAo 负责人信息
                    let rpcCompanyBusinessAo = {};
                    rpcCompanyBusinessAo = formData.orgBusiness;
                    rpcCompanyBusinessAo.businessAddress = formData.orgBusiness.address;
                    let rpcPersonChargeInfoAo = formData.rpcPersonChargeInfoAo;
                    rpcCompanyBusinessAo.companyName = formData.orgBusiness.orgName;
                    rpcCompanyBusinessAo.legalPerson = rpcPersonChargeInfoAo.personChargeName;
                    rpcCompanyBusinessAo.legalPhone = rpcPersonChargeInfoAo.personChargePhone;
                    rpcCompanyBusinessAo.legalIdNumber = rpcPersonChargeInfoAo.personChargeNo;
                    delete formData.orgBusiness.orgName;
                    delete formData.orgBusiness.address;
                    delete formData.orgBusiness;
                    delete formData.rpcPersonChargeInfoAo;
                    this.btnLoading = true;
                    // 机构所在地
                    formData.submitProvinceCode = this.branchComForm.cityAreaCodeList[0];
                    formData.submitCityCode = this.branchComForm.cityAreaCodeList[1];
                    // 业务城市
                    (formData.provinceCode = this.branchComForm.cityCodeList[0]), (formData.cityCode = this.branchComForm.cityCodeList[1]);
                    formData.areaCode = this.branchComForm.submitAreaCode;
                    formData.relationId = this.branchComForm.parentId;
                    formData.category = 2;
                    formData.orgType = 20;
                    let opt = {
                        data: {
                            rpcCompanyConsoleAo: formData,
        ...
        async getDetailData() {
            ...
            try {
                ...
                let allData = {
                    ...data.rpcCompanyConsoleAo,
                    submitAreaCode:data.rpcCompanyConsoleAo.areaCode,
                    cityAreaCodeList:[data.rpcCompanyConsoleAo.submitProvinceCode, data.rpcCompanyConsoleAo.submitCityCode],
                    cityCodeList: [data.rpcCompanyConsoleAo.provinceCode, data.rpcCompanyConsoleAo.cityCode],
                    rpcOrgBusinessAo: data.rpcCompanyBusinessAo,
                    rpcPersonChargeInfoAo: data.rpcPersonChargeInfoAo
                };
                ...
                this.branchComForm = allData;
                this.pageType = 'addBranchCompany';
        ...
        afterUpload(res, file, item) {
            try {
                ...
                this.$route.query.procInsId && (this.imageUpateKey = +new Date())
                // this.imgList.push({
        ...
        getMapInfo({ lat, lng, formattedAddress }) {
            this.branchComForm.longitude = lng;
            this.branchComForm.latitude = lat;
            this.branchComForm.address = formattedAddress ? formattedAddress : '';
        },
        //获取附件名称list-通用接口
        ...
        pickFormData(obj) {
          if (obj.rpcPersonChargeInfoAo === null) {
            obj.rpcPersonChargeInfoAo = Object.assign({}, branchComForm.rpcPersonChargeInfoAo)
          }

          if (obj.rpcCompanyConsoleAo) {
            obj.rpcCompanyConsoleAo.orgBusiness = Object.assign({}, branchComForm.orgBusiness)
          }

          if (obj.rpcCompanyBusinessAo === null) {
            obj.rpcCompanyBusinessAo = {}
          }

          if (obj.rpcCompanyPersonChargeAo === null) {
            obj.rpcCompanyPersonChargeAo = {}
          }
        }
    },
    beforeDestroy() {
