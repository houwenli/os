    ...
    <div class="requirenment-box">
        ...
        <div class="radioBox">
            <el-radio-group v-model="demandPermission" class="formRadio">
                <div>
                    <el-radio-button size="medium" label="1" v-if="getModulePower('/technologyDemand').demandApplyAllList"><span>所有需求</span></el-radio-button>
                </div>
                <div>
                    <el-radio-button size="medium" label="2" v-if="getModulePower('/technologyDemand').demandApplyList"><span>我发起的</span></el-radio-button>
                </div>
            </el-radio-group>
            <el-radio-group class="top_radio" v-model="demandStatus">
                <el-radio-button label="" size="medium"><span class="search-btn">全部</span></el-radio-button>
                <el-radio-button :label="0" size="medium"><span class="search-btn">待规划</span></el-radio-button>
                <el-radio-button :label="1" size="medium"><span class="search-btn">进行中</span></el-radio-button>
                <el-radio-button :label="2" size="medium">
                    <span class="search-btn">已完成 ({{ finishedCount || 0 }})</span>
                </el-radio-button>
                <el-radio-button :label="6" size="medium">
                    <span class="search-btn">已关闭 ({{ closedCount || 0 }})</span>
                </el-radio-button>
                <el-radio-button :label="5" size="medium">
                    <span class="search-btn">已挂起 ({{ hangCount || 0 }})</span>
                </el-radio-button>
            </el-radio-group>
        </div>
        <div class="right">
            <ws-table-el ref="table" v-bind="tableConfig" highlight-current-row :header-cell-style="{ background: '#F5F7FA' }" @select="tableSelect">
                ...
                <template>
                    ...
                    <ws-table-column-el prop="demandStatusName" label="需求状态">
                        <template slot-scope="scope" prop="demandStatus">
                            <el-tag type="warning" v-if="scope.row.demandStatus === 0">{{ scope.row.demandStatusName }}</el-tag>
                            <el-tag style="background-color: #ecf5ff; border-color: #d9ecff; color: #409eff" v-if="scope.row.demandStatus === 1">{{ scope.row.demandStatusName }}</el-tag>
                            <el-tag type="success" v-if="scope.row.demandStatus === 2">{{ scope.row.demandStatusName }}</el-tag>
                            <el-tag type="info" v-if="scope.row.demandStatus === 3 || scope.row.demandStatus === 4 || scope.row.demandStatus === 6">{{ scope.row.demandStatusName }}</el-tag>
                            <el-tag type="info" v-if="scope.row.demandStatus === 5">{{ scope.row.demandStatusName || '-' }}</el-tag>
                            <p v-if="scope.row.demandStatus === null">{{ '-' }}</p>
                        </template>
                    </ws-table-column-el>
                    <ws-table-column-el prop="projectName" label="关联项目名称">
                        <template slot-scope="scope">
                            {{ scope.row.projectName || '-' }}
                        </template>
                    </ws-table-column-el>
                    <ws-table-column-el prop="projectNodeName" label="项目节点">
                        <template slot-scope="scope">
                            {{ scope.row.projectNodeName || '-' }}
                        </template>
                    </ws-table-column-el>
                    <ws-table-column-el prop="startTime" label="项目开始时间">
                        <template slot-scope="scope">
                            {{ scope.row.startTime || '-' }}
                        </template>
                    </ws-table-column-el>
                    <ws-table-column-el prop="releaseTime" label="预计发版时间">
                        <template slot-scope="scope">
                    ...
                    <ws-table-column-el fixed="right" width="160" label="操作">
                        <template v-slot="scope">
                            <el-button type="text" style="color: #0088ff" @click="goToPage(scope.row)" v-if="getModulePower('/technologyDemand').detail">详情</el-button>
                            <el-button
                                type="text"
                                style="color: #0088ff"
                                v-if="(scope.row.demandStatus == 3 || scope.row.demandStatus == 4) && getModulePower('/technologyDemand').submit"
                                @click="hanldeResubmit(scope.row.id, 'cxtj', type)"
                            >
                                重新提交
                            </el-button>
                        </template>
...
export default {
    ...
    data() {
        return {
            ...
            statusCountOp: 'demandApplyStatusCount',
            tableConfig: {
                layout: 'flow',
                pageId: '1000000',
                pageCreateTime: '',
                ...
                lazyQuery: {
                    ...
                    op: {
                        selectList: 'demandApplyList',
                        selectCount: 'demandApplyCount',
                    },
    ...
    watch: {
        ...
        demandPermission(val) {
            if (val === '1') {
                ...
                this.tableConfig.lazyQuery.op.selectList = 'demandApplyAllList';
                this.tableConfig.lazyQuery.op.selectCount = 'demandApplyAllCount';
                this.statusCountOp = 'demandApplyAllStatusCount';
            }
            if (val === '2') {
                this.$refs.table.reset();
                this.tableConfig.lazyQuery.op.selectList = 'demandApplyList';
                this.tableConfig.lazyQuery.op.selectCount = 'demandApplyCount';
                this.statusCountOp = 'demandApplyStatusCount';
            }
            this.getDemandApplyStatusCount();
            this.demandStatus = '';
        },
    },
    activated() {
        console.log(this.getModulePower('/technologyDemand').demandApplyAllList,'chazhao');
        this.$nextTick(() => {
            this.demandStatus = '';
            this.$refs.table.reset();
        });
    ...
    methods: {
        ...
        //获取需求列表状态数量
        getDemandApplyStatusCount() {
            let data = {
                data: {},
                op: this.statusCountOp,
            };
            this.$axios(this.$getModuleUrl('/technologyDemand', this.statusCountOp), data).then(res => {
                const result = JSON.parse(res.data) || [];
                result.map(item => {
                    if (item.demandStatus === 2) {
                        this.finishedCount = item.count;
                    }
                    if (item.demandStatus === 5) {
                        this.hangCount = item.count;
                    }
                    if (item.demandStatus === 6) {
                        this.closedCount = item.count;
                    }
                });
            });
        },
