    ...
    <ws-table-el ref="table" v-bind="tableConfig" v-if="tableColumns.length" @sort-change="sortChange">
      <template #search>
        <search-group>
          ...
          <organization-all v-model="searchOptions.agencyNumber" :showSelect="false" orgType="20" ref="org" placeholder="请输入机构名称搜索" valueKey="agencyNumber">
          </organization-all>
        </search-group>
        <search-group>
        ...
        <search-group>
          <label for>等级：</label>
          <el-select v-model="searchOptions.gradeType" filterable clearable placeholder="请选择">
            <el-option v-for="subItem in levelListFilters" :key="subItem.id" :label="subItem.value" :value="subItem.id"></el-option>
          </el-select>
        </search-group>
      </template>
      ...
      <template>
        <ws-table-column-el :width="(titleObj.tip||titleObj.sortable)?'215':''" v-for="(titleObj,index) in tableColumns" :label="titleObj.title" :key="index" :prop="titleObj.key"
          ...
          <template slot-scope="scope">
            <span v-if="titleObj.key==='gradeType'">{{levelMap[scope.row[titleObj.key]]}}</span>
            <span v-else-if="titleObj.key==='vipAmount'">{{$baseMethod.regFenToYuan(scope.row[titleObj.key])}}</span>
            <span v-else-if="titleObj.add">{{`${scope.row[titleObj.key]}${titleObj.add}`}}</span>
