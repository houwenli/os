    ...
    <div class="form-wrap">
        <div class="fee-info">
            <div class="import_wrap">
                <div class="box">
                    <div class="content">
                        <div class="title">选择调整类型</div>
                        <div class="urbanCoFunderWrap">
                            <el-radio v-model="isUrbanCoFunder" label="0">删除合伙人</el-radio>
                            <el-radio v-model="isUrbanCoFunder" label="1">新增合伙人</el-radio>
                        </div>
...
export default {
    ...
    methods: {
        ...
        downLoad() {
            util.downloadFile('/static/xlsx/createPeople.xlsx', '合伙人表单');
        },
        start() {
            ...
            if (!this.isUrbanCoFunder) {
                this.$message.error('请选择调整类型');
                return false;
            }
            let formData = new FormData();
            formData.append('file', file);
            formData.append('data', JSON.stringify({ isUrbanCoFunder: this.isUrbanCoFunder }));
            formData.append('op', 'import');
            formData.append('token', this.$baseMethod.getStore('_ccmUser').token);
            this.$upLoad(this.$getModuleUrl(this.moduleName, this.op.import), formData)
                .then(res => {
                    this.progressType = 0;
                    this.dataResult = {
                        message: res.msg,
