    ...
    <div class="tableList">
        <ws-table-el :col="8" ref="table" v-bind="tableConfig" :header-cell-style="{ 'background-color': '#F5F7FA' }">
            <template #search>
                <search-group>
                    <label for>创建人：</label>
                    <el-input v-model.trim="searchOptions.createUserName" :maxlength="20" placeholder="请输入创建人" clearable></el-input>
                </search-group>
                <search-group>
                ...
                <search-group>
                    ...
                    <ws-date-range :startTime.sync="startTime" :endTime.sync="endTime"></ws-date-range>
                </search-group>
            </template>
            <template #options>
                <el-button-group v-if="powerList.getFiles" style="float: right">
            ...
            <template>
                <ws-table-column-el prop="fileType" label="文件类型" width="250" :formatter="sysTypeFormat"></ws-table-column-el>
                <ws-table-column-el prop="fileName" label="文件名称" width="350" show-overflow-tooltip></ws-table-column-el>
                <ws-table-column-el prop="createTime" label="创建时间" width="300">
                    <template slot-scope="scope">
                        {{ dateFormat(scope.row.createTime) }}
                    </template>
                </ws-table-column-el>
                <ws-table-column-el prop="createUserName" label="创建人" width="150"></ws-table-column-el>
...
export default {
    data() {
        return {
            ...
            tableConfig: {
                ...
                lazyQuery: {
                    moduleName: '/workbench/DJdownLoad',
                    op: {
                        selectList: 'getFiles'
                    },
    ...
    computed: {
        fileType() {
            ...
            const deleteKey = ['getFiles', 'download']
            const filterArr = []
    ...
    methods: {
        beforeSearch() {
            if (!this.searchOptions.type) {
                ...
                let startTime= this.startTime ? new Date(this.startTime).getTime() / 1000 : null;
                let endTime = this.endTime ? new Date(this.endTime).getTime() / 1000 : null;
                return { ...this.searchOptions, fileType,startTime,endTime };
            }
        },
        afterQuery(data) {
            data.body = data.body || []
        ...
        sysTypeFormat(row) {
            const sysTypeObj = this.fileType.find(item => item.id == row.fileType)
            return sysTypeObj.value
        },
        getModuleUrl(op) {
