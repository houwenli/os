    ...
    <div >
        <el-collapse v-model="activeNames">
            <el-collapse-item :title="formGroup.basicFormGroup.title" name="1">
                <form-group v-if="dataList.orderCharteredBaseInfoVo" :formGroup="formGroup.basicFormGroup | filterOneitme" :formData="dataList.orderCharteredBaseInfoVo"></form-group>
            </el-collapse-item>
            <el-collapse-item :title="formGroup.driverInfoFormGroup.title" name="3">
                <form-group v-if="dataList.orderCharteredBaseInfoVo" :formGroup="formGroup.driverInfoFormGroup" :formData="dataList.orderCharteredBaseInfoVo"></form-group>
            </el-collapse-item>
           <el-collapse-item :title="formGroup.realRideInfoFormGroup.title" name="2">
                <form-group v-if="dataList.rpcrealCharteredRideInfoVo" :formGroup="formGroup.realRideInfoFormGroup" :formData="dataList.rpcrealCharteredRideInfoVo"></form-group>
            </el-collapse-item>
...
<script>
    ...
    export default {
                ...
                formGroup: Object.freeze({
                    basicFormGroup: {
                        ...
                        attribute: [
                            ...
                            {label: '实际乘车人手机号', name: 'realPassengerPhone'},
                            {label: '实际乘车人姓名', name: 'realPassengerName'},
                            // {label: '套餐', name: 'setMeal'},
                            {label: '订单风险', name: 'orderType'},
                            ...
                            {label: '订单结束城市', name: 'endCityName'},
                            {label: '订单起点', name: 'orderStart'},
                            {label: '订单终点', name: 'orderEnd'},
                            ...
                            // {label: '预约出发时间', name: 'reserveStartTime'},
                        ]
                    },
                    driverInfoFormGroup:{
                        title:'车主信息',
                        ...
                        attribute: [
                            ...
                            {label: '运管类型', name: 'driverLevel', filters: 'driverLevelFilters'},
                            {
                                label: '系统合规类型',
                                name: 'driverLevel',
                            ...
                            {label: '包干类型', name: 'vipType', filters: 'vipFilters'},
                    ...
                    realRideInfoFormGroup: {
                        ...
                        attribute: [
                            ...
                            {label: '取消类型', name: 'cancelType'},
                            {label: '取消操作人', name: 'cancelUserName'},
        ...
        filters:{
          filterOneitme (obj) {
             let isOrder = _this.dataList.orderCharteredBaseInfoVo && _this.dataList.orderCharteredBaseInfoVo.orderChannel == 1 ? true : false  //是否自主下单
             let index = obj.attribute.findIndex(item=>{
              return item.name == 'passengerSysVersionAndPlatform'
             })
             let i =  obj.attribute.findIndex(item=>{
              return item.name == 'dispatchAgencyIdAndName'
             })
             if (index > 0 && isOrder && i < 0) {
               obj.attribute.splice(index+1,0,{label: '下单机构', name: 'dispatchAgencyIdAndName'})
             }
            return obj
          }
        }
