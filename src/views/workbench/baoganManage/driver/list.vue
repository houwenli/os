    ...
    <div class="bg-page" :class="isFolder?'addWidth':''">
        ...
        <ws-table-el align="left" ref="table" v-bind="tableConfig" :col="8">
            <template #search>
                ...
                <search-group>
                    <label for>多城市包干：</label>
                    <el-select v-model="searchOptions.isMultiCity" placeholder="请选择" clearable>
                        <el-option v-for="item in isMultiCityFilters" :key="item.id" :label="item.value" :value="item.id"></el-option>
                    </el-select>
                </search-group>
                <search-group>
            ...
            <template #options>
                ...
                <el-button type="primary" v-if="powerList.batchMatchUploading" @click="goMatch()">批量匹配</el-button>
            </template>
            <template>
                ...
                <ws-table-column-el align="center" prop="isMultiCity" label="多城市包干">
                    <template slot-scope="scope">{{scope.row.isMultiCity | isMultiCityFilters}}</template>
                </ws-table-column-el>
...
export default {
    ...
    methods: {
        batch() {
            this.$router.push({ name: 'driverBatchImport' });
        },
        goMatch() {
            this.$router.push({ name: 'batchMatch' });
        },
        detail(id) {
            this.$router.push({ name: 'driverBgForm', query: { id, isEncrypt: this.$refs.table.isEncrypt } });
        },
        beforeQuery() {
