    ...
    <div class="flex-column">
        <div class="form-wrap flex-1">
            <ws-descriptions title="基础信息">
                ...
                <ws-descriptions-item label="包干状态" :showTip="false">{{ getObj.status | bgFilters }}</ws-descriptions-item>
                    ...
                    <template>
                        ...
                        <ws-table-column-el align="center" label="操作" fixed="right">
                            <template slot-scope="scope">
                                <el-button
                                    type="text"
                                    v-if="powerList.give && scope.row.effictiveDays"
                                    style="color:#0088ff;"
                                    @click="adjust(scope.row,1)"
                                >调账</el-button>
                                <el-button
                                    type="text"
                                    v-if="powerList.setting"
                                    style="color:#0088ff;"
                                    @click="adjust(scope.row,2)"
                                >赠送</el-button>
                                <el-button
                                    type="text"
                                    v-if="scope.row.refundShow && powerList.setting"
                                    style="color:#0088ff;"
                                    @click="adjust(scope.row,3)"
                                >退款</el-button>
                            </template>
                        </ws-table-column-el>
                    </template>
...
export default {
    ...
    methods: {
        ...
        disabledDate(time) {
            return time.getTime() < Date.now() || time.getTime() > new Date(this.adjustData.vipEndTime).getTime() - 60*60*24*1000;
        },
        getDateTime(time) {
