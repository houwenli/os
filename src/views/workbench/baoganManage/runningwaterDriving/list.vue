<template>
    <div class="bg-page" :class="isFolder?'addWidth':''">
        ...
        <template v-if="centerDialogVisible">
            <operating-dialog v-model="centerDialogVisible" :title="title" :type="type" :info="dialogInfo" @updateList="search" :moduleName="moduleName"></operating-dialog>
        </template>
    </div>
</template>
...
export default {
    ...
    methods: {
        ...
        detail(id) {
            this.$router.push({ name: 'runningwaterDrivingForm', query: { id, isEncrypt: this.$refs.table.isEncrypt } });
        },
        formatAmount(m) {
        ...
        handleCommand(name) {
            switch (name) {
                case 'refundAudit': // 批量审核
                    ...
                    break;
                case 'batch': // 批量退款
                    this.$router.push({ name: 'runningwaterDrivingRefundBatch' });
                    break;
            }
        },
