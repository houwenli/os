<template>
    <div class="bg-page" :class="isFolder?'addWidth':''">
        ...
        <ws-table-el align="left" ref="table" v-bind="tableConfig" @selection-change="selectionChange" :col="8">
            <template #search>
                ...
                <search-group>
                    ...
                    <el-input
                        v-model.trim="searchOptions.tradeNo"
                ...
                <search-group>
                    ...
                    <el-input
                        v-model.trim="searchOptions.outTradeNo"
        ...
        <template v-if="centerDialogVisible">
            <operating-dialog v-model="centerDialogVisible" :title="title" :type="type" :info="dialogInfo" :payMode="payMode" @updateList="search"></operating-dialog>
        </template>
    </div>
</template>
...
export default {
    ...
    methods: {
        ...
        detail(id) {
            this.$router.push({ name: 'runningwaterForm', query: { id, isEncrypt: this.$refs.table.isEncrypt } });
        },
        formatAmount(m) {
        ...
        handleCommand(name) {
            switch (name) {
                case 'refundAudit': // 批量审核
                    ...
                    break;
                case 'batch': // 批量退款
                    this.$router.push({ name: 'refundBatchImport' });
                    break;
            }
        },
