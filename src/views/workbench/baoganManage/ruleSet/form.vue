        ...
        <div class="form-wrap flex-1">
                ...
                <div v-if="formData.settingStatus">
                    <div class="configBaogan">
                        <div class="inline-block">
                            ...
                            <span class="text" v-if="disabled">{{ formData.maxVipDay }}</span>
                            <el-input v-else v-model.trim="formData.maxVipDay" @blur="CheckData(regular[0], 'maxVipDay')" placeholder="【0, 20000】整数"></el-input>
                        </div>
                        <div class="inline-block">
            ...
            <div class="base-content">
                ...
                <div v-if="formData.limitStatus">
                    ...
                    <div v-for="(item, index) in formData.limitOptions" :key="index">
                        <div class="inline-block">
                            <label for class="label">车主入网时间：</label>
                            <span class="text" v-if="disabled">{{ item.firstAuditPassTimeLimit }}</span>
                            <el-date-picker
                                v-else
                                v-model="item.firstAuditPassTimeLimit"
                        ...
                        <div class="inline-block">
                            <label for class="label">系统合规类型：</label>
                            <span v-if="disabled">{{ item.certGroup }}</span>
                            <el-select v-else v-model="item.certGroup" placeholder="请选择" multiple collapse-tags>
                                <el-option v-for="item in driverTypeList" :key="item.id" :label="item.value" :value="item.id"></el-option>
                            </el-select>
                        </div>
                        <div class="operation-btn" v-if="!disabled">
                            <span @click="addNewItem('add')" class="subBtn" v-if="index === formData.limitOptions.length - 1">新增</span>
                            <span @click="deleteItem(index)" class="subBtn" style="color: #f05259" v-if="index > 0">删除</span>
                        </div>
                    </div>
                </div>
            </div>
...
export default {
    ...
    data() {
        return {
            ...
            formData: {
                ...
                maxAge:'',
                limitOptions: [{ firstAuditPassTimeLimit: '', certGroup: [] }],
                buyVipCity: ''
            },
            disabled: false,
            regular: [
            ...
            moduleName: '/workbench/baoganManage/ruleSet/list',
            enterQuery: null,
    ...
    computed: {
        ...mapState({
            ...
            driverTypeList: state => state.baogan.driverTypeList,
            buyVipCitys: state => state.baogan.buyVipCityFilters
        }),
        formContentStatus() {
            return {
                new: '新增',
    ...
    methods: {
        ...
        switchChange(val, type) {
            if (type === 1) {
                ...
                if (!val) {
                    ...
                    this.formData.limitOptions = [{ firstAuditPassTimeLimit: '', certGroup: [] }];
                }
            }
        },
        // 增加限购对象
        addNewItem(type) {
            const lastItem = this.formData.limitOptions[this.formData.limitOptions.length - 1];
            if (!lastItem.firstAuditPassTimeLimit) {
                this.$message.error('请先选择适购车主入网时间！');
                return false;
            } else if (!lastItem.certGroup.length) {
                this.$message.error('请先选择可购车主系统合规类型！');
                return false;
            }
            const { isTimeDup, isTypeDup } = this.checkDuplication();
            if (!isTimeDup) {
                this.$message.error('车主入网时间不允许重复');
                return false;
            } else if (!isTypeDup) {
                this.$message.error('可购车主系统合规类型不允许重复');
                return false;
            } else if (type === 'add') {
                this.formData.limitOptions.push({
                    firstAuditPassTimeLimit: '',
                    certGroup: [],
                });
            } else {
                return true;
        ...
        // 校验车主入网时间和可购车主系统合规类型不能重复
        checkDuplication() {
            let isTimeDup = false; // 时间是否重复
            let isTypeDup = false; // 类型是否重复
            let fullTime = [];
            let fullType = [];
            this.formData.limitOptions.forEach(({ firstAuditPassTimeLimit, certGroup }) => {
                fullTime.push(firstAuditPassTimeLimit);
                fullType.push(...certGroup);
            });
            isTimeDup = fullTime.length === [...new Set(fullTime)].length;
            isTypeDup = fullType.length === [...new Set(fullType)].length;

            return { isTimeDup, isTypeDup };
        },
        // 删除限购对象
        deleteItem(objIndex) {
            this.formData.limitOptions.splice(objIndex, 1);
        },
        // 获取详情信息
        getDetail() {
            let data = {
            ...
            this._axios(data,this.moduleName).then(res => {
                ...
                resultData.limitStatus &&
                    resultData.limitOptions.forEach(data => {
                        if (this.enterQuery.type == 'edit') {
                            data.certGroup = data.certGroup.split(',');
                        } else {
                            data.certGroup = this.convertCertGroup(data.certGroup);
                        }
                    });
                if (!resultData.limitStatus) {
                    resultData.limitOptions = [{ firstAuditPassTimeLimit: '', certGroup: [] }];
                }
        ...
        async save() {
            ...
            if (limitStatus && !this.addNewItem()) return;
            let op;
            let data = {
                scopeSwitch,
                settingStatus,
                ...
                buyVipCity,
                maxAge,
                limitOptions: limitOptions.map(({ firstAuditPassTimeLimit, certGroup }) => ({
                    firstAuditPassTimeLimit,
                    certGroup: certGroup.join(','),
                })),
            };
            if (cityCode || cityCodeList?.length) {
                // 全国无生效时间
