<!--
 * @Author: DaiYu
 * @Date: 2022-10-07 15:27:38
 * @LastEditors: DaiYu
 * @LastEditTime: 2022-10-08 15:57:39
 * @FilePath: \main\src\views\workbench\order-map\dispatch\cityMap2.0.vue
-->
<template>
...
const locaUrl = '//webapi.amap.com/loca?v=2.0&key=e12e6e76d6c6362c02efe99c2d9c4c5c';
let infoWin;
let infoWinData;
...
export default {
    ...
    watch: {
        showText(val) {
            // 切换文字图层
            this.setOptions(this.textLayer);
            let labelsLayer = this.textLayer.getLabelsLayer();
            try {
                // 无数据时蜂窝图层方法调用报错
                if (val) {
                    labelsLayer.show();
    ...
    beforeDestroy() {
        ...
        this.map?.off('click', this.onHexagonClick);
        this.myDestroy();
    },
    methods: {
        initMap() {
        ...
        setOptions(layer) {
            let colors = ['rgba(67, 67, 235, 0.68)', 'rgba(102, 253, 180, 0.68)', 'rgba(252, 208, 48, 0.68)', 'rgba(255, 137, 51, 0.68)', 'rgba(252, 88, 50, 0.68)'];
            let options = {
                // mode: 'count', // 聚合模式，可选值: sum(值求和)、max(最大值)、min(最小值)、mean(平均值)、median(中位数)、count(个数)
                unit: 'meter',
                topColor: (i, value) => {
                    let count = value.coordinates.length;
                    if (count < this.searchOptions.min) {
                        return 'rgba(14, 28, 75, 0.5)';
                    } else if (count < 5) {
                        return colors[0];
                    } else if (count < 15) {
                        return colors[1];
            ...
            if (this.showText) {
                options.label = {
                    text: {
                        content: (v, value) => {
                            return value.coordinates.length >= this.searchOptions.min ? value.coordinates.length + '' : '';
                        },
        ...
        createPolygonLayer(data) {
            ...
            this.districtLayer.setSource(geoSource);
            this.districtLayer.setStyle({
                topColor: 'rgba(14, 28, 75, 0.5)',
                // opacity: 0.5,
            });
            // this.loca.add(this.districtLayer)
            // this.districtLayer.render();
            // cityGeo = null;
        },
        // 渲染带文字的蜂窝图层
        // https://lbs.amap.com/api/loca-v2/api#hexagonlayer
        createTextHexagonLayer(heatmapData) {
            // this.loca.remove()
            if (!this.textLayer) {
                ...
                this.map.on('click', this.onHexagonClick);
                this.loca.add(this.textLayer);
            }
            // 清空文本标记
            let labelsLayer = this.textLayer.getLabelsLayer();
            labelsLayer.clear();
            let layers = this.map.getLayers().filter(l => l.CLASS_NAME == 'AMap.LabelsLayer')
            this.loca.remove(layers);
            // 设置蜂窝样式
            this.textLayer.setSource(heatmapData);
            this.setOptions(this.textLayer);
            console.log(labelsLayer.remove,labelsLayer, this.map.getLayers());
            // this.fitview(labelsLayer.getZooms());
            let zoom = this.map.getZoom();
            zoom < 11.5 && labelsLayer.hide();
            this.loading?.close();
            heatmapData = null;
        },
        zoomchange() {
        ...
        fitview(overlayList = []) {
            this.map.setFitView(overlayList, false, [60, 60, 60, 60], 13);
        },
        search() {
            // 清空面板数据
            this.centerPoint = {};
        ...
        async onHexagonClick(event) {
            console.log(event);
            let feat = this.textLayer.queryFeature(event.pixel.toArray());
            console.log(feat);
            if (!feat) return;
            if (feat.coordinates.length < this.searchOptions.min) return;
            try {
                let res = await this.geocoder(feat.center);
        ...
        // 地图消息弹窗
        openInfoWin(feat, { formattedAddress, addressComponent: { adcode } }) {
            if (!infoWin) {
            ...
            infoWin.open(this.map, infoWinData.lngLat);
        },
        // 弹窗关闭回调
        onInfoWinClose() {
            infoWinData = null;
        ...
        async getOrderPoints() {
            ...
            try {
                ...
                let geoFeatures = orderList.map(({ startLat, startLon }) => {
                    return {
                        type: 'Feature',
                        geometry: {
                            type: 'Point',
                            coordinates: [startLon, startLat], // lon, lat
                        },
                        properties: {
                            count: 1,
                        },
                ...
                heatmapData = new Loca.GeoJSONSource({
                    data: {
                        type: 'FeatureCollection',
                ...
                this.createTextHexagonLayer(heatmapData);
            } catch (error) {
                this.loading?.close();
            }
            // 渲染蜂窝数据
