    ...
    <div class="data_form">
        ...
        <ws-table-el v-else :span-method="arraySpanMethod" ref="table" v-bind="tableConfig" :data="tableData" :header-cell-style="{ background: '#F5F7FA' }">
            <template>
                ...
                <ws-table-column-el width="1100px" prop="areaNames" label="区">
                    <template slot-scope="scope">
                        {{ scope.row.areaNames ? scope.row.areaNames : '--' }}
                    </template>
                </ws-table-column-el>
            </template>
        </ws-table-el>
...
export default {
    ...
    methods: {
        ...
        async getCityList() {
            try {
                let { activityType, id, activityCityLimit, nameId } = this.formData;
                if (activityCityLimit === 0) return;
                let param = {
                    op: this.requestConfing ? this.requestConfing.op : 'getShowActivityCityCode',
                    activityType,
                    id,
                };
                let url = this.requestConfing ? this.requestConfing.url : nameId == 61 ? '/driverActivity/activityManager/select' : '/passengerAct/common/select';
                let res = await this._axios(param, '', url);
                if (res.code === 1) {
                    let list = res.data;
