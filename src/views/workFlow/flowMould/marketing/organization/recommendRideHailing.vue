<template>
    <el-form class="flow_form" :inline="true" label-width="140px" :model="dataObject">
        ...
        <!-- 纯tableUI展示组件 -->
        <showTable v-if="driverActiveConfigList.length" :columns="driverActiveConfigColumns" :list="driverActiveConfigList" title="车主奖励">
            <template #content>
                <div class="config-type-text">{{ showRewardTypeText(driverActiveConfigList[0]) }}</div>
            </template>
        </showTable>

        <showTable v-if="promoterConfigList.length" :columns="promoterConfigColumns" :list="promoterConfigList" title="推广员奖励">
            <template #content>
                <div class="config-type-text">{{ showRewardTypeText(promoterConfigList[0]) }}</div>
            </template>
        </showTable>
    </el-form>
</template>
...
export default {
  ...
  methods: {
    ...
    setCarActiveConfig () {
      ...
      let rewardType = list[0].rewardType

      let columns = driverActiveConfigColumns
      if (rewardType === 1) {
        driverActiveConfigColumns[0].label = '注册数量'
      } else {
        driverActiveConfigColumns[0].label = '累计推荐注册用户数量'
      }
    ...
    setPromoterConfig () {
      ...
      this.promoterConfigColumns = promoterConfigColumnsMap[rewardType]
